...
#TITLE: Limonium submodel script
#AUTHOR: Jacob Simon
...
#Establish lower elevation threshold for Limoniums
```{r}
limo.thresh<-ifelse(CNNdata$Limon > 0 & CNNdata$elevation..m. < 0.60, 1, 0)
#there are 2 instances of Limo presence below 0.60 m elevation (need to investigate this)

CNNdata$limothreshYN<-limo.thresh  #here also, 0 indicates hospitable habitat
```
#insert PA/log abundance columns and subset
```{r}
CNNdata$limoPA<-ifelse(CNNdata$Limon > 0,1,0)
CNNdata$logLimon<-log1p(CNNdata$Limon)
#now subset the dataframe
limo.hab<-subset(CNNdata,limothreshYN == 0 & saltpanYN == 0)
limo.inhosp<-rbind(subset(CNNdata,limothreshYN == 1),subset(CNNdata, saltpanYN == 1))
#sum to 9278?
sum(length(limo.hab[,1]),length(limo.inhosp[,1]))
#checks out
```
#conduct logistic regression on the PA data with the hospitable habitat df
```{r}
limoPAmod1<-glm(limoPA~elevation..m. + salinity..psu. + Juncus + Bor,family = 'binomial', data = limo.hab)

library(MASS)
limoStep<-stepAIC(limoPAmod1,direction = "both")
limoStep$anova
...
PA.resid<-resid(limoPAmod1)
```
#conduct loglinear regression on positive sarcocornia values
```{r}
limo.habY<-subset(limo.hab,limoPA==1)  #create a df for only obs where Limonium is present

limoPmod1<-lm(logLimon~elevation..m. + salinity..psu. + Juncus + Bor, data = limo.habY)

limoStep2<-stepAIC(limoPmod1,direction = "both")
limoStep2$anova

#final model selected is very close to both second place and initial model
limoPmod2<-lm(logLimon ~ elevation..m. + salinity..psu. + Juncus,data = limo.habY)
limoPmod3<-lm(logLimon ~ elevation..m. + Juncus,data = limo.habY) #final mod
```
#Assess
```{r}
#view selected model
...
#grab residuals for Moran's I testing
P.resid<-resid(limoPmod3)
```
#capture predictions for both models
```{r}
#create objects for loglinear prediction equation
elev<-limo.habY$elevation..m.
Junc<-limo.habY$Juncus

LLpred<- 0.973708 + 0.627516*elev + 0.011570*Junc 

limo.hab$limoLMpred<-0   
n<-0
for (i in 1:length(limo.hab[,1])) {   #index in the LM productivity estimates to sarc.hab
  test<-limo.hab$logLimon[i]
  if(test > 0 ){ #productivity estimate is zero where not present
    n<-n+1
    
    limo.hab$limoLMpred[i]<-LLpred[n]  
  }
}

#get logistic predictions from model
limo.hab$limoLOGpred<-predict(limoPAmod1,type = "response")

#insert 0 predictions for each value in the salt pan & below the elevation threshold
limo.inhosp$limoLOGpred<-0
limo.inhosp$limoLMpred<-0
```
#conduct spatial autocorrelation correction on both models
```{r}
library(spdep) #library the spatial dependence package

#carry out the Moran's I estimation for both models
PAxy.data<-cbind(limo.hab$easting..m.,limo.hab$northing..m.) 
Pxy.data<-cbind(limo.habY$easting..m.,limo.habY$northing..m.)
...
PA.limo.Moran<-moran.test(PA.resid,nb2listw(PAxy.nb,style ="W"))
P.limo.Moran<-moran.test(P.resid,nb2listw(Pxy.nb,style ="W"))
```

#compute adjusted sample size, standard errors and p values
```{r}
PAlimoNeff<-as.numeric(N.eff(PA.limo.Moran$estimate[1],length(limo.hab[,1]))) #calculate n' for each model using function written above
PlimoNeff<-as.numeric(N.eff(P.limo.Moran$estimate[1],length(limo.habY[,1])))
#n' values added to column 3 & 4 of Table 2

mod.adjuster(limoPAmod1,PAlimoNeff) #adjust each model and print values
mod.adjuster(limoPmod1,PlimoNeff)

#evaluate p values for increases above 0.05

#SOME P VALS NO LONGER SIGNIFICANT
```

#Merge predictions from the two models
```{r}
#bind the two dataframes together, and create a cohesive prediction value column
limo.merge<-rbind(limo.hab,limo.inhosp)

limo.merge$limoPRED<-limo.merge$limoLOGpred*limo.merge$limoLMpred
```
#regress our cohesive predictive value against the observed value
```{r}
limoFmod<-lm(limo.merge$logLimon ~ limo.merge$limoPRED)

summary(limoFmod)

#store R^2 value
limoR2<-summary(limoFmod)$r.squared
limoR2adj<-summary(limoFmod)$adj.r.squared
```
#calculate R2 and standardize coefficients
```{r}
limo_coef<-range.std.coef(limoPAmod1,limoPmod3,CNNdata)#calculate final range standardized coefficients
print(limo_coef)
```
