...
```{r}
PAsarcNeff<-as.numeric(N.eff(PA.sarc.Moran$estimate[1],length(sarc.hab[,1]))) #calculate n' for each model
...
```{r}
sarc_coef<-range.std.coef(sarcPAmod1,sarcPmod1,CNNdata)
```
#consolidate necessary info for graph

#check the fuctions performance on models with different variables
```{r}
elev<-mean(CNNdata$elevation..m.)
Bor<-mean(CNNdata$Bor)
Junc<-mean(CNNdata$Juncus)

maxElev<-max(CNNdata$elevation..m.)
maxBor<-max(CNNdata$Bor)
maxJunc<-max(CNNdata$Juncus)

minElev<-min(CNNdata$elevation..m.)
minBor<-min(CNNdata$Bor)
minJunc<-min(CNNdata$Juncus)

PAcoef<-sarcPAmod1$coefficients
Pcoef<-sarcPmod1$coefficients


maxElev<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*maxElev + PAcoef[4]*Bor + PAcoef[3]*Junc))))*(Pcoef[1] + Pcoef[2]*maxElev + Pcoef[3]*Junc + Pcoef[4]*Bor)
print(paste("max Elev toggle:",maxElev))

maxBor<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*elev + PAcoef[4]*maxBor + PAcoef[3]*Junc))))*(Pcoef[1] + Pcoef[2]*elev + Pcoef[3]*Junc + Pcoef[4]*maxBor)
print(paste("max Sal toggle:",maxBor))

maxJunc<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*elev + PAcoef[3]*maxJunc + PAcoef[4]*Bor))))*(Pcoef[1] + Pcoef[2]*elev + Pcoef[3]*maxJunc + Pcoef[4]*Bor)
print(paste("max Junc toggle:",maxJunc))


minElev<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*minElev + PAcoef[4]*Bor + PAcoef[3]*Junc))))*(Pcoef[1] + Pcoef[2]*minElev + Pcoef[3]*Junc + Pcoef[4]*Bor)
print(paste("min Elev toggle:",minElev))

minBor<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*elev + PAcoef[4]*minBor + PAcoef[3]*Junc))))*(Pcoef[1] + Pcoef[2]*elev + Pcoef[3]*Junc + Pcoef[4]*minBor)
print(paste("min Sal toggle:",minBor))

minJunc<-(1/(1+1/(exp(PAcoef[1] + PAcoef[2]*elev + PAcoef[4]*Bor + PAcoef[3]*minJunc))))*(Pcoef[1] + Pcoef[2]*elev + Pcoef[3]*minJunc + Pcoef[4]*Bor)
print(paste("min Junc toggle",minJunc))

diffElev<-maxElev - minElev
diffSal<-maxBor - minBor
diffJunc<-maxJunc - minJunc

rangeSarc<-range(CNNdata$logSarc)
  rangeSarc<-rangeSarc[2] - rangeSarc[1]
