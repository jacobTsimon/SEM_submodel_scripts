...
```
#Establish lower elevation threshold for Juncus
```{r}
junc.thresh<-ifelse(CNNdata$Juncus == 0& CNNdata$elevation..m. < 0.55, 1, 0)
#there are 74 instances of Juncus presence below 0.55 m elevation, but drawing the line here may be appropriate as i suspect some of these are misclassified Spartina (need to investigate this)

CNNdata$juncthreshYN<-junc.thresh  #here also, 0 indicates hospitable habitat
```
#insert PA/log abundance columns and subset
```{r}
CNNdata$juncPA<-ifelse(CNNdata$Juncus > 0,1,0)
CNNdata$logjunc<-log1p(CNNdata$Juncus)
#now subset the dataframe
junc.hab<-subset(CNNdata,juncthreshYN == 0 & saltpanYN == 0)
junc.inhosp<-rbind(subset(CNNdata,juncthreshYN == 1),subset(CNNdata, saltpanYN == 1))
#sum to 9278, checks out
sum(length(junc.hab[,1]),length(junc.inhosp[,1]))
```
#conduct logistic regression on the PA data with the hospitable habitat df
```{r}
juncPAmod1<-glm(juncPA~elevation..m. + salinity..psu.,family = 'binomial',data = junc.hab)

library(MASS)
juncStep<-stepAIC(juncPAmod1,direction = "both")
juncStep$anova
...
PA.resid<-resid(juncPAmod1)
```
#conduct loglinear regression on positive Juncus values
```{r}
junc.habY<-subset(junc.hab,juncPA==1)  #create a df for only rows with Juncus present

juncPmod1<-lm(logjunc~elevation..m. + salinity..psu.,data = junc.habY)

juncStep2<-stepAIC(juncPmod1,direction = "both")
juncStep2$anova
...
#grab residuals for Moran's I testing
P.resid<-resid(juncPmod1)
```
#capture predictions for both models
```{r}
#create objects for loglinear prediction equation
elev<-junc.habY$elevation..m.
sal<-junc.habY$salinity..psu.

LLpred<- -0.455652 + 2.430092*elev + 0.006245*sal

junc.hab$juncLMpred<-0   
n<-0
for (i in 1:length(junc.hab[,1])) {   #index in the LM productivity estimates to sarc.hab
  test<-junc.hab$logjunc[i]
  if(test > 0 ){ #productivity estimate is zero where not present
    n<-n+1
    
    junc.hab$juncLMpred[i]<-LLpred[n]  
  }
}


#get logistic predictions from model
junc.hab$juncLOGpred<-predict(juncPAmod1,type = "response")

#insert 0 predictions for each value in the salt pan & below the elevation threshold
junc.inhosp$juncLOGpred<-0
junc.inhosp$juncLMpred<-0
```
...
PAxy.data<-cbind(junc.hab$easting..m.,junc.hab$northing..m.) 
Pxy.data<-cbind(junc.habY$easting..m.,junc.habY$northing..m.)
...
PA.junc.Moran<-moran.test(PA.resid,nb2listw(PAxy.nb,style ="W"))
P.junc.Moran<-moran.test(P.resid,nb2listw(Pxy.nb,style ="W"))
```

#compute adjusted sample size, standard errors and p values
```{r}
PAjuncNeff<-as.numeric(N.eff(PA.junc.Moran$estimate[1],length(junc.hab[,1]))) #calculate n' for each model using function written above
PjuncNeff<-as.numeric(N.eff(P.junc.Moran$estimate[1],length(junc.habY[,1])))
#n' values added to column 3 & 4 of Table 2

mod.adjuster(juncPAmod1,PAjuncNeff) #adjust each model and print values
mod.adjuster(juncPmod1,PjuncNeff)
...
junc.merge<-rbind(junc.hab,junc.inhosp)

junc.merge$juncPRED<-junc.merge$juncLOGpred*junc.merge$juncLMpred
```
#regress our cohesive predictive value against the observed value to evaluate fit
```{r}
juncFmod<-lm(junc.merge$logjunc ~ junc.merge$juncPRED)

summary(juncFmod)

#store R^2 value
juncR2<-summary(juncFmod)$r.squared
juncR2adj<-summary(juncFmod)$adj.r.squared
```
#calculate full model R2 andstandardize coefficients
```{r}
junc_coef<-range.std.coef(juncPAmod1,juncPmod1,CNNdata)#calculate final range standardized coefficients
print(junc_coef)
```
